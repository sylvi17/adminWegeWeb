            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-10 text-center text-red-400">
                  {error}
                  <button onClick={onRefresh} className="ml-2 underline text-teal-500">Coba lagi</button>
                </td>
              </tr>
            ) : data.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">Tidak ada data guru.</td></tr>
            ) : (
              data.map((g) => (
                <tr key={g.id} className="border-t border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-3">
                    <button className="text-xs border border-gray-200 rounded-lg px-3 py-1 text-gray-600 hover:bg-gray-100 transition">Detail</button>
                  </td>
                </tr>
              ))
            )}
      ...
      <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button onClick={() => onPageChange(page - 1)} disabled={page === 1}
            className="text-xs px-3 py-1 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40">‹</button>
          <button className="text-xs px-3 py-1 rounded-lg bg-teal-600 text-white">{page}</button>
