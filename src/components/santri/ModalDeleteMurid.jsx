...
export default function ModalDeleteMurid({ murid, onClose, onSuccess }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(json.message || "Gagal menghapus murid");
      }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-[18px] shadow-2xl w-full max-w-sm mx-4">
        <div className="px-6 py-6 text-center">
          ...
          <h2 className="text-[1rem] font-extrabold text-[#1a1a1a] mb-2">Hapus Murid</h2>
          <p className="text-sm text-[#999]">
            Apakah kamu yakin ingin menghapus{" "}
            <span className="font-bold text-[#333]">{murid.nama}</span>?
